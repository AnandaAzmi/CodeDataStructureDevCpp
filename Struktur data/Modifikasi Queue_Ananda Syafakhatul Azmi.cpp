...
struct Queue //membuat struct untuk antrian
{
 ...
 void Enqueue (int data, string name) //fngsi untuk memasukan data ke dalam antrian (Modifikasi bagian string name)
 {
    if (IsEmpty()==1){//jika antrian kosong
        ...
        antrian.name[antrian.tail]= name; //Modifikasi
    ...
    if (IsFull()==0){ //jika antrian sudha berisi dan belum penuh
        ...
        antrian.name [antrian.tail]=name;
...
int dequeue () //fungsi untuk menghapus data dari antrian
{
    ...
    string n =antrian.name[antrian.head];
    for (int i = antrian.head; i<=antrian.tail-1; i++) //perulangan untuk menggeser data dari belakang ke depan
    {
        ...
        antrian.name[i]= antrian.name [i+1]; 
    }
...
void tampil()//fungsi untuk menampilkan data dalam antrian
{
    if (IsEmpty()==0)//jika antrian tidak kosong
    {
        ...
        {
            cout << endl;
            cout <<"Nomor antrian   : "<<antrian.data[i]<<endl; //tampilkan data (Modifikasi bagian antrian.name dan pesan nomor antrian dan nama)
            cout <<"Nama            : "<<antrian.name [i]<<endl;
        }
    }
...
int main ()//program utama antrian
{
    ...
    do //perulangan
    {
        ...
        {
        case 1:
        cout<<"Masukan Nomor Antrian  : "; cin >>data; cout<<endl;
        cout<<"Masukan Nama           : ";cin >>nama; cout <<endl;//modifikasi
            Enqueue(data,nama); //modifikaasi
            break;
